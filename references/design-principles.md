...
20. **Two valid slide modes — pick one per deck and stay in it.** *Presenter mode*: target ≤15 words/slide (hard limit 25 — see §8), image-led, sparse. *Document mode*: dense, scannable, may include short bullets — but still hierarchical. Never mix in the same deck. [Tufte vs. Reynolds — synthesis]
...
- **Presenter mode (live):** target ≤15 words per slide, hard limit 25, image-led, headline + visual, presenter speaks the detail. (Two numbers on purpose: 15 is the design target Reynolds argues for, 25 is the threshold the automated check rejects at — aim for 15, fail at 26.)
